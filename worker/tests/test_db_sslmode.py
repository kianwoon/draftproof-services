...
def test_neon_url_without_ssl_or_options_gets_both():
    assert _neon_connect_kwargs(NEON) == {"sslmode": "require", "options": EP}


def test_existing_sslmode_kept_options_added():
    assert _neon_connect_kwargs(NEON + "?sslmode=verify-full") == {"options": EP}


def test_existing_options_kept_sslmode_added():
    assert _neon_connect_kwargs(NEON + "?options=" + EP) == {"sslmode": "require"}


def test_both_present_no_extra():
    assert _neon_connect_kwargs(NEON + f"?sslmode=require&options={EP}") == {}


def test_non_neon_host_gets_sslmode_only():
    assert _neon_connect_kwargs("postgresql://u:p@db.internal.example.com:5432/app") == {"sslmode": "require"}


def test_empty_url_forces_sslmode_only():
    assert _neon_connect_kwargs("") == {"sslmode": "require"}
