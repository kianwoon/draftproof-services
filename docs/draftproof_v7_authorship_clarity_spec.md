...
- **`ai_paraphrased` is not separable from single-document evidence — two measured rejections.** (1) The MiniLM cosine family (semantic monotony) was gate-rejected: best effective AUC 0.669 < 0.70 vs `ai_generated_like` (`paraphrase_feature_study.json`). (2) The V8 interaction family's winner (`paraphrase_mismatch` = specificity_student_evidence × detector, effective AUC 0.7202 vs generated, ESL-clean) PASSED its study gate but is **tuning-infeasible against acceptance**: para ≥40% has no candidate; para ~33% is buyable only by collapsing `ai_assisted_polished` 87%→25% — polished and paraphrased are variants of the same human essay differing only in edit depth, and no single-doc signal measured to date grades edit depth (`v8_frontier_result.json`). Do not retry single-doc families without a genuinely new evidence theory. **Resolution shipped:** three-way display fallback (`ai_transformed` merges the paraphrased+generated indicators where indistinguishable; four-way data retained; tier verdict remains the authority). **OWNER CONSTRAINT (hard, 2026-07-08, reiterated): users scan ONE copy at a time — no earlier-draft/paired-upload workflow, ever. Do not propose it again.** The three-way display IS the single-copy resolution. **Phase C CLOSED (2026-07-08).** C1 (per-sentence deep-scan score mixture shape, cache replay): FAIL — the desklib checkpoint SATURATES on LLM-touched text (lightly-polished docs: zero sentences below 0.2, mean sentence score 0.99 — identical profile to fully-paraphrased), so no edit-depth information exists at this detector's resolution (`phase_c1_mixture_study.json`). C2 (function/content-word perplexity asymmetry): CANCELLED by owner with concurring evidence — the industry has abandoned perplexity as a decider (modern detectors are trained transformer classifiers; perplexity survives only as an ensemble first-layer), our composite already carries GPT-2 perplexity/top-k features that do not separate the walls, and a function-word variant targets exactly the ESL-divergent token class the FPR gates exist to protect. **Single-copy 4-way attribution is closed at today's technology: the three-way display is the standing answer.** The only credible future route is a purpose-trained edit-depth classifier (labeled pair corpus + fine-tune; real cost; unproven anywhere in the industry) — documented option, no commitment.
- The `paraphrase_mismatch` signal stays wired (17 signals) at a display-neutral starter weight — verified identical confusion matrix to the promoted state; it is tuner-ready data for V8b.
