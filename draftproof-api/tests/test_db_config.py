...
def test_koyeb_postgres_url_gets_asyncpg_scheme():
    url = _normalize_database_url(f"postgres://u:p@{KOYEB_HOST}/koyebdb")

    assert url == f"postgresql+asyncpg://u:p@{KOYEB_HOST}/koyebdb"


def test_existing_sslmode_is_removed_from_asyncpg_sqlalchemy_url():
